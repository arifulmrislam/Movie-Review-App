...
export const createReview = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const existingReview = await RR.findOne({
            where: { movie_id: Number(movie_id), user_id: Number(user_id) }
        });

        if (existingReview) {
            res.status(403).json({ error: "You have already reviewed this movie" });
            return;
        }
